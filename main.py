# main.py — Carrinho Abandonado → WhatsApp (UAZAPI v2)
import os
import logging
...
UAZAPI_CONVERT = os.getenv("UAZAPI_CONVERT", "true").strip().lower()  # "true" recomendado
...
MSG_TEMPLATE: str = os.getenv(
    "MSG_TEMPLATE",
     "Te achei {first_name}!"
     "Sou Iara, consultora de vendas da Paginatto e vi que você deixou o {product} no carrinho..."
     "Porque voce desistiu?🤔"
     "Segue link para retormar a compra: {cart_url}" ,
)

# Endpoint correto do UAZAPI v2 (uazapiGO)
UAZAPI_SEND_TEXT_URL = lambda: f"{UAZAPI_SERVER_URL}/send/text"
...
def resolve_checkout_url(payload: dict, scoped: Optional[dict] = None) -> str:
    s = scoped or {}
    keys = ["checkout_link", "checkout_url", "cart_url", "recovery_url", "recover_url", "abandoned_checkout_url"]
    for k in keys:
        if s.get(k):
            return s[k]
    for k in keys:
...
def _product_from_item(item: dict) -> str:
    ...
    title = _coalesce(
        [
            ...
            prod_obj.get("title"),
            " ".join(
                t for t in [item.get("title"), item.get("variant_title") or variant.get("title")] if t
            ),
        ]
    )
...
def _price_from_item_or_totals(item: dict, totals: List[Any]) -> str:
    raw = _coalesce([item.get("price"), item.get("unit_price"), item.get("line_price"), item.get("subtotal"), item.get("total")])
    if raw in (None, "", 0):
...
def parse_abandoned_payload(payload: dict) -> dict:
    ...
    price = _price_from_item_or_totals(first, [data.get("total_line_items_price"), data.get("subtotal_price"), data.get("total_price")])
...
def parse_order_payload(payload: dict) -> dict:
    ...
    name = (customer.get("name") or customer.get("full_name") or f"{customer.get('first_name','')} {customer.get('last_name','')}".strip() or "cliente")
    first_name = customer.get("first_name") or (name.split()[0] if name else "cliente")

    first = items[0] or {}
    product = _product_from_item(first)
    price = _price_from_item_or_totals(first, [order.get("total_line_items_price"), order.get("subtotal_price"), order.get("total_price"), order.get("unformatted_total_price")])
...
async def uazapi_send_text(chat_id: str, text: str) -> Dict[str, Any]:
    """
    UAZAPI v2:
    ...
    if not UAZAPI_SERVER_URL:
        return {"status": "error", "body": "missing_env:UAZAPI_SERVER_URL"}
    if not UAZAPI_INSTANCE_TOKEN:
        return {"status": "error", "body": "missing_env:UAZAPI_INSTANCE_TOKEN"}

    url = UAZAPI_SEND_TEXT_URL()
    headers = {
        "Content-Type": "application/json",
        "token": UAZAPI_INSTANCE_TOKEN,
        "convert": "true" if UAZAPI_CONVERT != "false" else "false",
    }
    ...
    try:
        ...
        log.exception(f"Erro UAZAPI send_text: {e}")
