...
def parse_order_payload(payload: dict) -> dict:
    ...
    return {
        ...
        "checkout_url": order.get("checkout_url") or order.get("cart_url"),
        "cart_url": order.get("cart_url") or order.get("checkout_url"),
...
# Se não tiver setada, fica None; tudo bem porque vamos usar safe_format
...
class _SafeDict(dict):
    def __missing__(self, key):
        # se o placeholder não existir, retorna vazio em vez de quebrar
        return ""


def safe_format(template: Optional[str], data: Dict[str, Any]) -> str:
    """Formata sem levantar KeyError quando faltar alguma chave."""
    if not template:
        # fallback simples caso alguém tenha apagado MSG_TEMPLATE
        template = "Olá {name}! {product} por {price}. Link: {checkout_url}"
    return template.format_map(_SafeDict(data))


def resolve_checkout_url(payload: dict, order: Optional[dict] = None) -> str:
    """Tenta achar o link de checkout em todos os lugares comuns."""
    order = order or {}
    return (
...
def parse_abandoned_payload(payload: dict) -> dict:
    ...
    # Formata preço
    if isinstance(price, (int, float)):
        price = f"R$ {float(price):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    ...
    return {
        ...
        "payment_status": None,
        "payment_method": None,
        # guardamos nos dois nomes para o template não quebrar
...
async def cartpanda_webhook(payload: Dict[str, Any] = Body(...)):
    ...
    if ("abandoned" in event) or ("checkout.abandoned" in event):
        ...
        # Monta dados para o template (com as duas chaves de link)
        data = {
